...
for slug,name,tagline,desc,sections,img,related in SERVICES:
    ...
    if slug=='wall-cleaning':body+='<p class="related">更多案例：'+link('/cases/fubon-xihua','西華富邦')+'、'+link('/cases/lien-jade-oriental','聯聚東方')+'</p>'
    if slug=='waterproof':body+='<p class="related">實際案例：'+link('/cases/metrowalk-sealant','大江購物中心矽利康更新')+'、'+link('/cases/guomei-star','國美之星屋頂接縫防水')+'</p>'
    body+='</div>'
...
FEATURED=json.loads((ROOT/'scripts/featured_cases.json').read_text())
for c in FEATURED:
    photos=c['photos']
    body='<div class="crumb">'+link('/portfolio','施工案例')+' / '+e(c['name'])+'</div>'+intro('SELECTED WORK',c['name'],c['desc'])
    body+='<dl class="facts"><div><dt>案場</dt><dd>'+e(c['location'])+'</dd></div><div><dt>施作工項</dt><dd>'+e(c['service'])+'</dd></div></dl>'
    body+='<div class="gallery">'+''.join('<figure><img src="/images/cases/'+e(f)+'" alt="'+e(alt)+'" loading="lazy"><figcaption>'+e(alt)+'</figcaption></figure>' for f,alt in photos)+'</div>'
    body+='<div class="reading">'+section('現場紀錄',c['desc'])+'<p class="source-note">資料來源：東方繩洗案場照片。'+('資料夾紀錄日期：'+c['date']+'。' if c['date'] else '')+'照片呈現本案施作位置，實際委託範圍依個別案場確認。</p><p class="related">'+link('/services/'+c['route'],'了解'+('外牆清洗' if c['route']=='wall-cleaning' else '防水工程')+'服務')+'</p></div>'
    write_page('/cases/'+c['slug'],c['name']+c['service']+'案例',c['desc'],body,image='/images/cases/'+photos[0][0])
portfolio=intro('SELECTED WORK' ,'留下每個案場的工作紀錄。','從現場照片、工項與施工紀錄，了解東方繩洗的實際作業。')+'<div class="grid">'+''.join(card('/cases/'+c['slug'],c['name'],c['service']+' · '+c['location'],'/images/cases/'+c['photos'][0][0]) for c in FEATURED)+''.join(card('/cases/'+c[0],c[1],c[4]+' · '+c[2],'/images/cases/'+c[6][0][0]) for c in CASES)+'</div>'
...
write_page('/portfolio','施工案例', '西華富邦、聯聚東方、大江購物中心、國美之星與森聯摩天41等案場的清洗、防水與維護照片。',portfolio)
