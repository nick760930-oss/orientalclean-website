...
NAV='<header class="site-header"><a class="brand" href="/" aria-label="東方繩洗首頁"><img src="/images/logo-horizontal-dark.png" alt="東方繩洗 ORIENTAL CLEAN" width="210" height="70"></a><nav aria-label="主要選單">'+''.join(link(p,t) for p,t in [('/','首頁'),('/services','工程服務'),('/portfolio','施工案例'),('/journal','工程筆記'),('/contact','聯絡我們')])+'</nav></header>'
...
for slug,title,loc,period,service,desc,photos,sections in CASES:
    ...
    body+='<div class="gallery">'+''.join(f'<figure><img src="/images/cases/{f}" alt="{e(alt)}" loading="lazy" width="1200" height="900"><figcaption>{e(alt)}</figcaption></figure>' for f,alt in photos)+'</div><div class="reading">'+''.join(section(*s) for s in sections)+'<p class="source-note">資料來源：東方繩洗2025年度本案成果報告。公開頁僅整理案場、工項與施工照片。</p><p class="related">'+link('/services/wall-cleaning','了解外牆清洗服務')+' · '+link('/journal/facade-cleaning-quote','清洗報價比較重點')+'</p></div>'
